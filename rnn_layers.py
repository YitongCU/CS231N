...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  h = np.zeros((N,T,H))
  for i in range(T):
    hs[i], _ = rnn_step_forward(x[:,i,:], hs[i-1], Wx, Wh, b)
    h[:,i,:] = hs[i]
  cache = (x,h0,Wx,Wh,hs)
  ##############################################################################
  #                               END OF YOUR CODE                             #
...
def rnn_backward(dh, cache):
  ...
  for i in reversed(xrange(T)):
    ...
    dx[:,i,:], dh0, dtmp_Wx, dtmp_Wh, dtmp_b = rnn_step_backward(dh[:,i,:]+dh0, t_cache)
    db += dtmp_b
    dWx += dtmp_Wx
    dWh += dtmp_Wh
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  N,D = x.shape
  H = prev_h.shape[1]
  # bigW = np.concatenate((Wx.T,Wh.T),axis = 1) ## bigW: (4H * (D+H))
  # bigInput = np.concatenate((x.T,prev_h.T),axis = 0) ## bigInput: ((D+H) * N)
  # tmp = (np.dot(bigW,bigInput).T+b).T
  # i = sigmoid(tmp[:H,:])
  # f = sigmoid(tmp[H:2*H,:])
  ...
  # cache = (x, prev_h, prev_c, Wx, Wh, b, tmp, i, f, o, g, next_c, next_h)
