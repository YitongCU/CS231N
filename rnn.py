...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    ############################################################################
    h0 = features.dot(W_proj)+b_proj ## transform the image feature to h0 state ##
    N,T = captions_in.shape
    x,cache0 = word_embedding_forward(captions_in,W_embed)

    if self.cell_type == 'rnn':
        h,cache1 = rnn_forward(x,h0,Wx,Wh,b) # h: (N,T,H)
    else:
        h,cache1 = lstm_foward(x,h0,Wx,Wh,b)

    out,cache2 = temporal_affine_forward(h,W_vocab,b_vocab) #out: (N,T,V)
  ...
  def sample(self, features, max_length=30):
    ...
    for i in range(max_length):
        if self.cell_type == 'rnn':
            ...
            next_h,cache = lstm_step_forward(x,next_h,Wx,Wh,b)

        score = np.dot(next_h,W_vocab)+ b_vocab
        captions[:,i] = np.argmax(score,1)
